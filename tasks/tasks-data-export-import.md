...
**IMPORTANT:** Change `- [x]` to `- [x]` as you complete tasks.
...
- [x] 0.0 Create feature branch
  - [x] 0.1 Run `git checkout -b feature/data-export-import`
  - [x] 0.2 Run `npm test` to confirm all 62 existing tests pass before touching anything.

- [x] 1.0 Build `backupService.ts` — export, import, validation, and merge logic
  - [x] 1.1 Create `src/services/backupService.ts` with the following pure functions:
  ...
  - [x] 1.2 Define and export the `NooksBackup` TypeScript interface in `backupService.ts`:
    ```ts
    ...
    ```
  - [x] 1.3 Write extensive unit tests in `src/services/backupService.test.ts`:
  ...
  - [x] 1.4 **Verify & Test:** Run `npm test` and `npm run lint`. All tests must pass, no type errors.
  - [x] 1.5 **Human Review:** Confirm task 1 is complete before proceeding.
  - [x] 1.6 **Local Commit:** `git commit -m "feat: add backupService with export, import, validation, and merge logic"`

- [x] 2.0 Update `nudgeService.ts` to include backup-overdue nudge
  - [x] 2.1 Update the `generateNudges` function signature to accept an optional second parameter: `lastExportDate: Date | null`
  - [x] 2.2 Add logic: if `lastExportDate` is null OR is more than 3 days ago, push a new nudge:
    - `id: 'backup-overdue'`
    - `type: 'gentle'`
  ...
  - [x] 2.3 Update `nudgeService.test.ts` with new tests:
  ...
  - [x] 2.4 **Verify & Test:** Run `npm test` and `npm run lint`. All tests must pass.
  - [x] 2.5 **Human Review:** Confirm task 2 is complete before proceeding.
  - [x] 2.6 **Local Commit:** `git commit -m "feat: add backup-overdue nudge to nudgeService"`

- [x] 3.0 Build `SettingsView.tsx`
  - [x] 3.1 Create `src/views/SettingsView.tsx` with:
  ...
  - [x] 3.2 Build the **choice modal** (can reuse existing `Modal` component):
    - Title: "Import Backup"
  ...
  - [x] 3.3 Write unit tests in `src/views/SettingsView.test.tsx`:
  ...
  - [x] 3.4 **Verify & Test:** Run `npm test` and `npm run lint`. All tests pass.
  - [x] 3.5 **Human Review:** Confirm task 3 is complete before proceeding.
  - [x] 3.6 **Local Commit:** `git commit -m "feat: add SettingsView with export and import UI"`

- [x] 4.0 Wire Settings into App navigation and update HomeView
  - [x] 4.1 Add `'settings'` to the `TabType` union in `App.tsx` (or handle as an overlay — whichever is cleaner given the existing nav structure)
  - [x] 4.2 Add a gear icon (use `Settings` from `lucide-react`) to the `HomeView` header that navigates to Settings
  - [x] 4.3 Update `HomeView` to call `generateNudges(tasks, getLastExportDate())` so the backup-overdue nudge is passed through
  - [x] 4.4 When the backup-overdue nudge card is tapped on Home, navigate to Settings
  - [x] 4.5 **Verify & Test:** Run `npm test` and `npm run lint`. All tests pass.
  - [x] 4.6 **Human Review:** Confirm task 4 is complete before proceeding.
  - [x] 4.7 **Local Commit:** `git commit -m "feat: wire Settings into App nav and connect backup nudge to HomeView"`

- [x] 5.0 Finalise & Cleanup
  - [x] 5.1 **Architecture Sync:** Update `ARCHITECTURE.md` — add `backupService.ts` to the Business Logic Layer section, add `SettingsView.tsx` to the View Layer section, document the `localStorage` key `nooks_last_export`
  - [x] 5.2 Run `npm run test:coverage` — confirm >85% overall and >80% for all modified/new files
  - [x] 5.3 **Human Review:** Ask user to do a manual smoke test: export → clear data → import → verify data is restored
  - [x] 5.4 **PR Request:** Ask the user if a PR should be created.
  - [x] 5.5 **Push:** Push the local branch to remote upon approval.
  - [x] 5.6 **Cleanup:** Once PR is merged, delete local and remote branches, remove `tasks/prd-data-export-import.md` and `tasks/tasks-data-export-import.md`.
